...
- (BOOL) toFront{
    ...
    BOOL succeed = [self changePosition:AVCaptureDevicePositionFront];
    if (succeed) {
...
- (BOOL) toBack{
    ...
    BOOL succeed = [self changePosition:AVCaptureDevicePositionBack];
    if (succeed) {
...
- (BOOL) connect{
    ...
    captureVideoConnection.videoOrientation = _currentOrientation;
    
    // 设置是否为镜像，前置摄像头采集到的数据本来就是翻转的，这里设置为镜像把画面转回来
    if (_currentPosition == AVCaptureDevicePositionFront && captureVideoConnection.supportsVideoMirroring){
